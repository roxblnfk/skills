...
final readonly class InitRunner
{
    ...
    private function writeStub(string $target, IOInterface $io): bool
    {
        ...
        // Spec §6.3: stub `skills.json` ships with the `local` and
        // `remote` knobs visible so users discover them without
        // reading docs. `local.composer: true` is also the default,
        // but we emit it explicitly — hiding it would make the npm
        // / go toggles seem surprise-feature-y when they arrive.
